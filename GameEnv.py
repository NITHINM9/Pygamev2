...
class Pedestrian:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.radius = 8  # Define the pedestrian size
    ...
    def move(self):
        # Simulate random movement: move in one of four directions
        directions = ["up", "down", "left", "right"]
        direction = random.choice(directions)

        step = 5  # Increase the step size to make the movement more noticeable
...
class RacingEnv:
    def __init__(self):
        ...
        self.pedestrians = []  # List to hold pedestrians
    ...
    def check_pedestrian_collision(self):
        for pedestrian in self.pedestrians:
            if distance(myPoint(self.car.x, self.car.y), myPoint(pedestrian.x, pedestrian.y)) < pedestrian.radius + self.car.width // 2:
                return True
        return False
    ...
    def step(self, action):
        ...
        # Check if car collides with pedestrians
        if self.check_pedestrian_collision():
            reward += PENALTY
            done = True
    ...
    def handle_mouse_click(self, pos):
        # Spawn a pedestrian at the clicked position
        self.pedestrians.append(Pedestrian(pos[0], pos[1]))
        # Slow down the car when a pedestrian appears
        self.car.vel = max(self.car.vel - 2, 1)